...
const Profile = ({ doctorData }) => {
    ...
    const handleExperinecesChange = (event, index) => {
        handleReusableInputChangeFunc('experiences', index, event)
    }
    const deleteExperineces = (e, index) => {
        e.preventDefault()
    ...
    const addtimeSlot = e => {
        ...
        addItem('timeSlots', {
            day: 'Sunday', startingTime: '10:00', endingTime: '4:30',
        })
    }
        ...
        <div>
            ...
            <form>
                ...
                <div className='mb-5'>
                    ...
                    <button onClick={addQualification} className='bg-[#000] py-2 px-5 rounded text-white h-fit cursor-pointer'>Add Quafilication</button>
                </div>
                <div className='mb-5'>
                    ...
                    {formData.experiences?.map((item, index) => <div key={index}>
                        <div>
                            <div className='grid grid-cols-2 gap-5'>
                                <div>
                                    <p className='form__label'>Starting Date*</p>
                                    <input onChange={e => handleExperinecesChange(e, index)} type="date" name='startingDate' value={item.startingDate} className='form__input' />
                                </div>
                                <div>
                                    <p className='form__label'>Ending Date*</p>
                                    <input onChange={e => handleExperinecesChange(e, index)} type="date" name='endingDate' value={item.endingDate} className='form__input' />
                                </div>
                            </div>
                            <div className='grid grid-cols-2 gap-5 mt-5'>
                                <div>
                                    <p className='form__label'>Position*</p>
                                    <input onChange={e => handleExperinecesChange(e, index)} type="text" name='position' value={item.position} className='form__input' />
                                </div>
                                <div>
                                    <p className='form__label'>Hospital*</p>
                                    <input onChange={e => handleExperinecesChange(e, index)} type="text" name='hospital' value={item.hospital} className='form__input' />
                                </div>
                            </div>

                            <button onClick={e => deleteExperineces(e, index)} className='bg-red-600 p-2 rounded-full text-white text-[18px] mt-2 mb-[30px] cursor-pointer'>
                                <AiOutlineDelete />
                            </button>
                        </div>
                    ...
                    <button onClick={addExperineces} className='bg-[#000] py-2 px-5 rounded text-white h-fit cursor-pointer'>Add Experience</button>
                </div>
                <div className='mb-5'>
                    ...
                    <button onClick={addtimeSlot} className='bg-[#000] py-2 px-5 rounded text-white h-fit cursor-pointer mt-6'>Add TimeSlot</button>
                </div>
